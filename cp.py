...
def replace_variables():
    ...
    undo_variables = []
    watch_variables = []
    color_vals_to_keep = ['97m', '97m', '97m', '97m', '97m', '97m']
...
def get_keywords():
    ...
    @rtype: list
    """
    kw = ["class", "print", "require", "#noinspection", "except", "del", "return", "with", "super", "catch", " pass", "switch", "raise", "for", "when", "if", "elif", "else", "while", "finally", "try", "unless", "catch", "$on", "$("]
    return kw
...
def init_file(args):
    ...
    if args.myfile:
        ...
        print "cp.py:1301", "no -f (file) given as argument"
        exit(1)
...
def main():
    ...
    for line in sio_file2:
        line = line.replace("1597", str(num))
        num += 1
        buffer_string += line
